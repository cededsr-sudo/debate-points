...
function reasonProfile(analysis) {
  const joined = (analysis.sentences || []).join(" ").toLowerCase();
  ...
  const opinionHits = countHits(joined, OPINION_WORDS);
...
function heuristicAIRefinement(teamAAnalysis, teamBAnalysis) {
  ...
  return {
    strongestArgumentSide: strongest.winner.sideName,
    strongestArgument:
      strongest.winner.bestSentence || strongest.winner.truth || strongest.winner.main_position,
    whyStrongest:
